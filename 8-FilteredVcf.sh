...
bcftools norm -m -any -f $REF vcf/11x7_family.vcf.gz \
  ...
  | bcftools filter -e "FMT/DP[$P1]<6 | FMT/DP[$P2]<6" -Oz \
      -o vcf/11x7_filtered.vcf.gz
tabix -p vcf vcf/11x7_filtered.vcf.gz
bcftools view -H vcf/11x7_filtered.vcf.gz | wc -l

# Quick check DP < 6, every row: both columns >=6
bcftools query -s 7-AD,11-AD -f '[%DP\t]\n' vcf/11x7_filtered.vcf.gz | head -10

# For OneMap
# 7x11
bcftools view -s 11-AD,7-AD,2-7x11-LV,3-7x11-LV vcf/7x11_filtered.vcf.gz \
  -Oz -o vcf/7x11_individuals.vcf.gz && tabix -p vcf vcf/7x11_individuals.vcf.gz
# 11x7 (note: different larvae — B and C)
bcftools view -s 11-AD,7-AD,C-11x7-LV vcf/11x7_filtered.vcf.gz \
  -Oz -o vcf/11x7_individuals.vcf.gz && tabix -p vcf vcf/11x7_individuals.vcf.gz



scp dmflores@stampede3.tacc.utexas.edu:/scratch/08717/dmflores/ofav-wgs-large/vcf/\*individuals.vcf.gz .